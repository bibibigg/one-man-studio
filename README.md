...
### 기타
- Google 소셜 로그인 (Supabase Auth + PKCE)
- 완성 영상 공유 링크 생성
- 카테고리·장르별 AI 프롬프트 자동 최적화

### 인증
Supabase Auth 기반 Google OAuth 로그인. PKCE 플로우로 보안을 강화했으며, `@supabase/ssr`을 통해 쿠키 기반 세션을 서버/클라이언트 컴포넌트 모두에서 공유. Next.js 미들웨어에서 `getUser()`로 토큰을 서버에서 직접 재검증해 비로그인 사용자의 보호 경로 접근을 차단.

→ 상세 내용: [docs/auth.md](./docs/auth.md)
...
- 상세 설정: [docs/git-strategy.md](./docs/git-strategy.md#claude-code-자동-코드-리뷰)
...
Google OAuth Client ID/Secret은 Supabase Dashboard → Authentication → Providers → Google에서 설정합니다.
→ 인증 설정 상세: [docs/auth.md](./docs/auth.md)
